...
class Trader:
    ...
    def _sell_market(self, quantity) -> int:
        ...
        mn_index = 0
        for _ in range(len(self.markets)):
            if self.markets[_].order_book['bid'].last and self.markets[mn_index].order_book['bid'] and \
                    self.markets[_].order_book['bid'].last.price > self.markets[mn_index].order_book['bid'].last.price:
                mn_index = _
